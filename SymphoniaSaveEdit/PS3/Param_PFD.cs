...
namespace PS3FileSystem
{
    public class Param_PFD
    {
        ...
        {
            byte[] buffer = PFDHashTable.Buffer;
        ...
        {
            byte[] buffer = PFDEntrySignatureTable.Buffer;
            buffer = Functions.GetHMACSHA1(realkey, buffer, 0, buffer.Length);
            return buffer;
        }
        ...
        private void DoProgress(string message, MessageType type)
        {
            if (ProgressChanged != null)
                ProgressChanged(this, message, type);
        }
        ...
        /// <summary>
        ///     A PFD Entry contains child file info
        /// </summary>
            ...
            internal byte[] EntryData
            {
                get
                {
                    ...
                    using (var bw = new BinaryWriter(ms))
                    {
                        ...
                        for (int i = 0; i < file_hashes.Count; i++)
        ...
        {
            public static ulong capacity = 0;
            public static ulong num_reserved = 0;
            public static ulong num_used = 0;
            ...
            public static byte[] Buffer
            {
                get
                {
                    ...
                    using (var bw = new BinaryWriter(ms))
                    {
                        ...
                        foreach (ulong value in entries)
        ...
        /// <summary>
        ///     Initialize new PARAM.PFD Instance
        /// </summary>
        public Param_PFD()
        {
        }
        ...
        ///     Initialize new PARAM.PFD Instance
        /// </summary>
        /// <param name="input">System.IO.Stream for Param.pfd</param>
        /// nput">
        /// </param>
        ...
        /// <summary>
        ///     Check if Top hash is valid, fix if needed
        /// </summary>
        ...
        /// <summary>
        ///     validate all Empty bottom hashes and fix it if needed
        /// </summary>
        ...
        {
            byte[] buffer = GetDefaultHash();
            if (buffer == null)
                return false;
            var indexes = new List<int>();
            foreach (PFDEntry ent in PFDEntries.entries)
                indexes.Add((int) CalculateHashTableEntryIndex(ent.file_name));
            for (int i = 0; i < PFDEntrySignatureTable.hashes.Count; i++)
            {
        ...
        /// <summary>
        ///     Check if File CID2 is valid, fix if needed
        /// </summary>
        /// <param name="fix">set 'true' if you wish to fix it if hash is invalid</param>
        ...
        /// <summary>
        ///     Check if entry hash is valid, fix if needed
        /// </summary>
        ...
        ///     Check if entry hash is valid, fix if needed
        /// </summary>
        /// <param name="filepath">the filepath of the file that is  specified inside the Param.PFD file entries</param>
        /// <param name="fix">set true if you wish to fix the file entry hash if invalid</param>
        /// <returns>true if file entry hash is valid</returns>
        ...
        /// <summary>
        ///     Check if all entry hashes are valid, and fix them if needed
        /// </summary>
        /// <param name="rootdirectory">The root directory where the PARAM.PFD and al its child files are located</param>
        /// <param name="fix">set true if you wish to fix all invalid entry hashes</param>
        /// <returns>
        ///     true if all file entry hashes are valid<</returns>
        public bool ValidAllEntryHashes(string rootdirectory, bool fix)
        {
            for (int i = 0; i < PFDEntries.entries.Count; i++)
            {
                string filepath = rootdirectory + "\\" + PFDEntries.entries[i].file_name;
                if (!File.Exists(filepath))
                    return false;
        ...
        /// <summary>
        ///     Check if All Param.PFD Hashes are valid,and Fix them if needed
        /// </summary>
        /// <param name="rootdirectory">The root directory where the PARAM.PFD and al its child files are located</param>
        /// <param name="fix">set true if you wish to fix all invalid hashes</param>
        ...
        /// <summary>
        ///     Rebuild the Param.PFD with all changes made
        /// </summary>
        ...
        /// <summary>
        ///     Combine and get the loaded PARAM.PFD file as byte array
        /// </summary>
        ...
        public bool Decrypt(ref byte[] inputdata, string entryname)
        {
            try
            {
                byte[] data = Decrypt(new MemoryStream(inputdata), entryname);
                if (data == null)
                    return false;
        ...
        {
            byte[] data = DecryptToBytes(filepath);
            if (data == null)
                return null;
        ...
        {
            byte[] data = EncryptToBytes(filepath);
            if (data == null)
                return null;
            return new MemoryStream(data);
        }
