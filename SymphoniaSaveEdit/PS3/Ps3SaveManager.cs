    ...
    public class Ps3SaveManager
    {
        ...
        public int EncryptAllFiles()
        {
            try
            {
                if (Param_PFD == null || !Directory.Exists(RootPath))
                    return -1;
                int x = Param_PFD.EncryptAllFiles(RootPath);
                if (x > 0)
        ...
        private byte[] GetSecureFileIdFromConfigFile(string titleid)
        {
            if (GameConfigList == null || GameConfigList.Length == 0)
                return null;
            return (from i in GameConfigList from s in i.GameIDs where s.ToLower() == titleid.ToLower() where i.SecureFileID != null && i.SecureFileID.Length == 32 select i.SecureFileID.StringToByteArray()).FirstOrDefault();
        }
    }
}